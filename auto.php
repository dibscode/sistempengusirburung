...
if (!$conn->connect_error) {
  ...
  $q = $conn->query("SELECT id, filename, waktu FROM foto WHERE jenis='otomatis' ORDER BY id DESC LIMIT 60");
  if ($q) {
...
<main class="max-w-md mx-auto px-4 pt-4 pb-24">
  <header class="flex items-center justify-between">
    <div>
      <div class="text-sm font-semibold text-slate-900">Gambar Otomatis</div>
      <div class="mt-1 text-xs text-slate-500">Menampilkan foto dengan jenis = otomatis</div>
    </div>
    <div class="rounded-2xl border border-slate-200 bg-white px-3 py-2 text-right">
      <div class="text-[11px] text-slate-500">Total</div>
      <div class="text-lg font-semibold text-blue-700"><?= $totalOtomatis ?></div>
    </div>
  </header>
      ...
      <div class="mt-3 grid grid-cols-2 gap-3">
        <?php foreach ($rows as $r) { ?>
          <div class="group relative rounded-2xl border border-slate-200 bg-white overflow-hidden">
            ...
            >
              <img src="uploads/<?= htmlspecialchars($r['filename']) ?>" class="h-32 w-full object-cover" loading="lazy" alt="Foto otomatis">
            </button>
            <div class="p-3">
...
<nav class="fixed bottom-0 left-0 right-0 border-t border-slate-200 bg-white">
  <div class="mx-auto max-w-md px-3">
    <div class="grid grid-cols-3 py-2">
      ...
      <a href="auto.php" class="flex flex-col items-center justify-center gap-1 rounded-xl px-2 py-2 text-blue-700">
        <div class="relative">
          <svg class="h-6 w-6" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
            <path fill-rule="evenodd" d="M12 2.25c-5.385 0-9.75 4.365-9.75 9.75S6.615 21.75 12 21.75 21.75 17.385 21.75 12 17.385 2.25 12 2.25Zm.75 5.25a.75.75 0 0 0-1.5 0v4.5c0 .414.336.75.75.75h3a.75.75 0 0 0 0-1.5h-2.25V7.5Z" clip-rule="evenodd" />
          </svg>
